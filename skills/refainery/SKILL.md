...
1. Verify `mnemonai` headless support.
   ...
   - Rebuild only on a real version signal — the binary rejects `--json`, `--since`, or `--cwd`, or `"$MNEMONAI_BIN" --version` is older than `0.12.3`. Then build an updated `mnemonai` from source in a temporary directory and set `MNEMONAI_BIN` to that checkout's `target/debug/mnemonai`. See `references/mnemonai-json.md` for the exact commands.
