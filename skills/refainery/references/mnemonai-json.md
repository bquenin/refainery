...
If that command fails with an unexpected `--json`, `--since`, or `--cwd` argument, or `"$MNEMONAI_BIN" --version` reports a version older than `0.12.3`, the installed `mnemonai` is too old. Build an updated binary from source in a temporary checkout and use it for the session:
...
"$MNEMONAI_BIN" show "$session" --json |
  jq '.messages[]
    ...
    | (
        ...
        and .tool_result_error == null
        and .tool_result_status == null
        and $normalized_exit_code == null
