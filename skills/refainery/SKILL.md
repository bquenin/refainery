...
1. Verify `mnemonai` headless support.
   ...
   - Run `"$MNEMONAI_BIN" list --json --since 7d --cwd . --limit 1`, `"$MNEMONAI_BIN" search --help`, and `"$MNEMONAI_BIN" show --help` before starting analysis. Confirm that `search` exists and `show` lists `--grep`; the retrieval workflow relies on both features.
   - Load one session with `"$MNEMONAI_BIN" show <id-or-path> --json` and confirm `messages[]` is ordered, every message carries `index` and `entry_index`, tool calls carry `tool_call_id` and `tool_name`, and results expose a matching `tool_call_id` when the provider records one. The `tool_result_status`, `tool_result_exit_code`, and `tool_result_error` fields are best-effort: they appear only when the provider exposes them, so their absence is normal and is not a sign of an old binary.
   - Rebuild only on a real version signal — the binary rejects the required commands or flags, or `"$MNEMONAI_BIN" --version` is older than `0.15.0` by semantic version (compare numerically, so `0.15.10` is newer than `0.15.0`, not older). Then build an updated `mnemonai` from source in a temporary directory and set `MNEMONAI_BIN` to that checkout's `target/debug/mnemonai`. See `references/mnemonai-json.md` for the exact commands.
   - If a checkout cannot be built (no network, no toolchain), stop and report that the skill needs `mnemonai` 0.15.0 or newer for headless search and focused session retrieval.
...
2. Define the scope from the user's request.
   ...
   - Use `list` for an unbiased time-window audit. Its previews do not search full conversation content, so do not use preview filtering to find a named skill, tool, command, or failure pattern.
   - For a named topic, build one concrete keyword set (artifact, command, skill, error string) and one abstract set (the underlying problem class). Search each with `"$MNEMONAI_BIN" search <words...> --since 30d --limit 20 --snippets 2 --json`. Search words are case-insensitive substring matches joined with AND; use distinctive words and vary synonyms when a query returns no useful hits.
   - Add `--cwd .` only when the topic is repo-local, and add `--provider`, explicit time bounds, or an explicit session path when the user supplies them. When the current session ID is known, pass `--exclude-session <id>` so the active incomplete trace cannot dominate the candidates.

3. Load candidate sessions.
   - Start with `"$MNEMONAI_BIN" show <id-or-path> --grep <pattern> --context 2 --json` to inspect why a search result matched without loading the full transcript into context. Repeat `--grep` for OR patterns; direct matches carry `matched: true`, context neighbors do not, and original message indices are preserved.
   - Treat focused output as a review aid only. It can omit a distant paired call/result or recovery message, so use full `show <id-or-path> --json` or the bundled evidence helper before reconstructing a complete trace or writing a finding.
   - Search results omit `preview` and `parse_errors`; inspect the selected session's `conversation` object before trusting its evidence.
